...
public class EarthShatterProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		for (int index0 = 0; index0 < (int) Math.ceil((3 + ((LivingEntity) entity).getAttribute(ForgeRegistries.ATTRIBUTES.getValue(new ResourceLocation("additional_attributes:spell_school_nature"))).getValue())
				* ((LivingEntity) entity).getAttribute(ForgeRegistries.ATTRIBUTES.getValue(new ResourceLocation("irons_spellbooks:nature_spell_power"))).getValue()); index0++) {
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = JoygModEntities.EARTH_SPIKE.get().spawn(_level, BlockPos.containing(x + Mth.nextDouble(RandomSource.create(), -4, 4), y, z + Mth.nextDouble(RandomSource.create(), -4, 4)), MobSpawnType.MOB_SUMMONED);
